...
export const jobsAPI = {
  ...
  search: async (searchParams) => {
    ...
    return await apiRequest(`/companies/search?${queryString.toString()}`);
  },

  getSuggestions: async (query) => {
    return await apiRequest(`/companies/suggestions?q=${encodeURIComponent(query)}`);
  },

  getFilterOptions: async () => {
    return await apiRequest('/companies/filters');
  },

  create: async (jobData) => {
    return await apiRequest('/companies/add', {
      method: 'POST',
  ...
  getByAdmin: async (adminEmail) => {
    return await apiRequest(`/companies/${adminEmail}`);
  },
};
